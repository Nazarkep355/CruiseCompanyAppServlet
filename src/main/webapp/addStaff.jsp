...
<html>
<head>
    <title><fmt:message bundle="${bundle}" key="AddStaff"/> </title>
...
<div class="col-md-10 mx-auto col-lg-5"
    ...
    <form action="/controller" method="post" class="p-4 p-md-5 border rounded-3 bg-light">
        <input name="controller" value="addStaff" type="hidden">
        <small class = "text-muted" style="margin-top: 15px; color: darkcyan;text-align: center"><fmt:message bundle="${bundle}" key="AddStaff"/></small>
        <div class="form-floating mb-3">
            <input type="text" name="name" class="form-control" id="nameInput" placeholder="example"
                   pattern="[А-Яа-яA-Za-zіІїЇєЄ.0-9_\- ]{2,50}">
            <label for="floatingInput"><fmt:message bundle="${bundle}" key="Name"/></label>
        </div>
        <div class="form-floating mb-3">
            <input type="text" name="position" class="form-control" id="posInput" placeholder="example"
                   pattern="[А-Яа-яA-Za-zіІїЇєЄ.0-9_\- ]{2,50}">
            <label for="floatingInput"><fmt:message bundle="${bundle}" key="Position"/></label>
        </div>
        <button class="w-100 btn btn-lg btn-primary" style="width: 150px;" type="submit">
            <fmt:message bundle="${bundle}" key="AddStaff"/>
        </button>
        <c:if test='${error!=null&&error.equals("EmployeeAlreadyAdded")}'>
            <small class = "text-muted" style="margin-top: 15px; color: red">
                <fmt:message bundle="${bundle}" key="EmployeeAlreadyAdded"/>
            </small>
        </c:if></form>
</div>
  ...
  height: 80px;">
    <ul class="nav justify-content-center border-bottom pb-3 mb-3">
        <form action="/controller" method="post">
            ...
            <input type="hidden" name="prev" value="/controller?controller=addStaffPage">
