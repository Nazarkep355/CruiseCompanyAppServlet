...
<div class="col-md-10 mx-auto col-lg-5"
    ...
    <form action="/controller" method="post" class="p-4 p-md-5 border rounded-3 bg-light">
        <input name="controller" value="addPort" type="hidden">
        ...
        <div class="form-floating mb-3">
            <input type="text" name="city" class="form-control" id="floatingInput" placeholder="example"
                   pattern="[А-Яа-яA-Za-zіІїЇєЄ.0-9_\-]{2,50}">
            <label for="floatingInput"><fmt:message bundle="${bundle}" key="EnterCity"/></label>
        </div>
        <button class="w-100 btn btn-lg btn-primary" style="width: 150px;" type="submit">
            <fmt:message bundle="${bundle}" key="AddPort"/>
        </button>
        <c:if test='${error!=null&&error.equals("PortInDB")}'>
            <small class = "text-muted" style="margin-top: 15px; color: red">
                <fmt:message bundle="${bundle}" key="PortInDB"/>
            </small>
        </c:if></form>
</div>
  ...
  height: 80px;">
    <ul class="nav justify-content-center border-bottom pb-3 mb-3">
        <form action="/controller" method="post">
            ...
            <input type="hidden" name="prev" value="/controller?controller=addPortPage">
